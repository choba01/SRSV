...
static struct timespec *trenutakObrade;
    ...
    while (nijeKraj)
    {
        for (int i = 0; i < brUlaza; i++)
        {
            ...
            if (zatrazenaPromjenaUlaza == 1 )
            {
                ...
                struct timespec trenutakZavrsetkaObrade;
                clock_gettime(CLOCK_MONOTONIC, &trenutakZavrsetkaObrade);
                trenutakObrade[indeksPromjeneUlaza] = trenutakZavrsetkaObrade;
                /* printf("%d: Tocno vrijeme kad je obrada gotova: tv_sec = %ld, tv_nsec = %ld\n", 
                i, trenutakZavrsetkaObrade.tv_sec, trenutakZavrsetkaObrade.tv_nsec); */
...
void *ulaznaDretva(void *arg)
{
    ...
    int novaPerioda = perioda;
    int k = 1;
    while (nijeKraj)
    {
        //while((ukupnoNeobradeno+1)/(ukupnoPromjena+1) < 0.3){
        

        novaPerioda = k * perioda + ulazniParametri[id][6];
        ...
        if (novaPerioda > ulazniParametri[id][3]){
        ...
        while (!ulazniParametri[id][5] && (izracunajVrijemeUMilisekundama(pocetakRada, trenutnoVrijeme) < trenutakPrvePojave + novaPerioda))
        {
            usleep(10000);
            clock_gettime(CLOCK_MONOTONIC, &trenutnoVrijeme);
        }

        if ((vrijemeReakcijeUlaza[id].tv_sec != 0 || vrijemeReakcijeUlaza[id].tv_nsec != 0) )//odgovor primljen od upravljaca
        {
            //printf("Dretva %d\n",id);
            ...
            if(ulazniParametri[id][5]==1 && izracunajVrijemeUMilisekundama(trenutakPromjeneStanja, trenutakObrade[id]) < novaPerioda){//obrada upravljaca gotova

               /*  printf("%d: Tocno vrijeme kad je obrada gotova: tv_sec = %ld, tv_nsec = %ld\n", 
                id, trenutakObrade[id].tv_sec, trenutakObrade[id].tv_nsec); */
            ...
            if(ulazniParametri[id][5]==1 && izracunajVrijemeUMilisekundama(trenutakPromjeneStanja, trenutakObrade[id]) > novaPerioda)
            {
                //printf("Dretva %d\n",id);
        ...
        if( ukupnoPromjena > 0 && (double) ukupnoNeobradeno /  (double) ukupnoPromjena >= 0.3){
            
            nijeKraj=0;

        }
        
        trenutakPrvePojave += novaPerioda;
        ulazniParametri[id][6] -= ulazniParametri[id][3] * k*10;
        k++;
            //}
...
int main(int argc, char *argv[])
{
    ...
    trenutakObrade = malloc(brUlaza * sizeof(struct timespec));
    memset(trenutakObrade, 0, brUlaza * sizeof(struct timespec));
    ...
    for (int i = 0; i < brUlaza; i++)
    {
        ...
        ulazniParametri[i][6] = ulaz[i][0];     //vrijednost za koju skracujemo, pocetno na period
    }
